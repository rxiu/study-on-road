 ...
 * KEEDIO
 */
package com.rxiu.flume.source.ftp.source;

import com.rxiu.flume.source.ftp.client.factory.SourceFactory;
import com.rxiu.flume.source.ftp.client.KeedioSource;
import com.rxiu.flume.source.ftp.client.filters.KeedioFileFilter;
import com.rxiu.flume.source.ftp.metrics.SourceCounter;
import com.rxiu.flume.source.ftp.source.utils.FTPSourceEventListener;
...
public class Source extends AbstractSource implements Configurable, PollableSource {
    ...
    if (!(list.isEmpty())) {
      ...
      for (T element : list) {
        ...
        if (keedioSource.isDirectory(element)) {
          ...
          keedioSource.getExistFileList().add(dirToList + "/" + elementName + "/" + keedioSource.getModifiedTime(element));  //control of deleted files in server

          //test if file is new in collection
          if (!(keedioSource.getFileList().containsKey(dirToList + "/" + elementName + "/" + keedioSource.getModifiedTime(element)))) { //new file
            ...
            long prevSize = (long) keedioSource.getFileList().get(dirToList + "/" + elementName + "/" + keedioSource.getModifiedTime(element));
            position = prevSize;
            long dif = keedioSource.getObjectSize(element) - (long) keedioSource.getFileList()
              .get(dirToList + "/" + elementName + "/" + keedioSource.getModifiedTime(element));

            if (dif > 0) {
              LOGGER.info("Modified: " + elementName + " ,size: " + dif);
            } else if (dif < 0) { //known and full modified
              keedioSource.getExistFileList().remove(dirToList + "/" + elementName + "/" + keedioSource.getModifiedTime(element)); //will be rediscovered as new file
              keedioSource.saveMap();
              continue;
            } else {
              continue;
          ...
          try {
            ...
            if (success) {
              processMessage("".getBytes(), elementName, dirToList);
              keedioSource.getFileList().put(dirToList + "/" + elementName + "/" +  keedioSource.getModifiedTime(element), keedioSource.getObjectSize(element));
              keedioSource.saveMap();
  ...
  private void processMessage(byte[] lastInfo, String fileName, String filePath, int position) {
    ...
    if (message.length == 0) {
      ...
      headers.put("pos", "reading");
    }
    headers.put("fileName", fileName);
    headers.put("rootPath", filePath);
    headers.put("filePath", String.join("/", filePath, fileName));
    headers.put("filePathMd5", DigestUtils.md5Hex(filePath));
  ...
  private void processMessage(byte[] lastInfo, String fileName, String filePath) {
    this.processMessage(lastInfo, fileName, filePath, -1);
  }
